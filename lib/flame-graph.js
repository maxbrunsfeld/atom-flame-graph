...
module.exports = {
  ...
  async startTracing() {
    ...
    traceProcess.on('close', (code) => {
      ...
      fs.readFile(traceOutputFile.path, 'utf8', (error, output) => {
        if (error) {
          atom.notifications.addWarning('Capturing stacks failed', {detail: error.message})
          return
        }

        const aggregator = new StackAggregator(2000, containingFunctionName);
